...
- `deploy.yml`: Builds and deploys the web app (PWA) to Cloudflare Workers on every push, excluding dependency update branches.
  ...
  On all other pushes, builds without telemetry and runs publishes a _preview_ Cloudflare Worker version.
  If the target branch is not `main` or `dev`, the PWA module is disabled so users' disks aren't filled by service-worker caches for URLs they'll never revisit.
