...
export const TabsTheme = tv({
  slots: {
    ...
    indicator: `
      ...
      transition-[translate,width,height] duration-200
    `,
    trigger: `
      relative inline-flex shrink-0 cursor-pointer items-center gap-1.5
      rounded-md font-medium text-muted
      focus-visible:outline-2 focus-visible:outline-inverted
      disabled:cursor-not-allowed disabled:opacity-75
      data-[state=active]:text-inverted
    `,
    label: "truncate",
    content: `
      w-full
  ...
  variants: {
    orientation: {
      horizontal: {
        root: "flex-col",
        indicator: `
          inset-y-1 left-0 w-(--reka-tabs-indicator-size)
      ...
      vertical: {
        list: "flex-col",
        indicator: `
          inset-x-1 top-0 h-(--reka-tabs-indicator-size)
