...
<template>
  ...
  >
    <template v-for="(group, groupIndex) in items" :key="`group-${groupIndex}`">
      <Menubar.Separator v-if="groupIndex > 0" :class="ui.separator()" />

      <Menubar.Group :class="ui.group()">
        <template v-for="(item, index) in group" :key="`group-${groupIndex}-${index}`">
          <Menubar.Separator v-if="item.type === 'separator'" :class="ui.separator({ class: item.class })" />

          <Menubar.Label v-else-if="item.type === 'label'" :class="ui.label({ class: item.class })">
            {{ item.label }}
          </Menubar.Label>
          ...
          <Menubar.Sub v-else-if="item.children?.length">
            <Menubar.SubTrigger :disabled="item.disabled" :class="ui.item({ class: item.class })">
