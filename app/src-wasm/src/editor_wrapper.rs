...
impl std::fmt::Display for OpenPatternResult {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "OpenPatternResult(id={}, title={})", self.id, self.title)
  }
}
...
impl EditorWrapper {
  ...
  /// Adds a Borsh-serialized `EmbroiderlyProject` and persists it without a file handle.
  #[wasm_bindgen(js_name = "addPattern")]
  pub async fn add_pattern(&self, data: Vec<u8>) -> Result<OpenPatternResult, Error> {
    self.add_pattern_impl(data).await
  }
  ...
  pub async fn open_pattern_from_data(&self, data: &[u8], file_name: &str) -> Result<OpenPatternResult, Error> {
    self.open_pattern_from_data_impl(data, file_name).await
  }
  ...
  /// Returns the Borsh-serialized `EmbroiderlyProject`.
  ...
  pub async fn load_pattern(&self, project_id: &str) -> Result<Vec<u8>, Error> {
    self.load_pattern_impl(project_id).await
  }
  ...
  pub async fn save_pattern(
    ...
    self.save_pattern_impl(project_id, file_handle.map(Into::into)).await
  }
  ...
  pub async fn export_pattern(
    ...
    self.export_pattern_impl(project_id, file_handle.into()).await
  }
  ...
  pub async fn close_pattern(&self, project_id: &str, force: bool) -> Result<(), Error> {
    self.close_pattern_impl(project_id, force).await
  }

  /// Adds a stitch to the pattern layer.
  #[wasm_bindgen(js_name = "addStitch")]
  pub async fn add_stitch(&self, project_id: &str, layer_index: u32, stitch_data: &[u8]) -> Result<(), Error> {
  ...
  pub async fn update_fabric(&self, project_id: &str, fabric_data: &[u8]) -> Result<(), Error> {
    self.update_fabric_impl(project_id, fabric_data).await
  }

  /// Updates grid properties.
  #[wasm_bindgen(js_name = "updateGrid")]
  pub async fn update_grid(&self, project_id: &str, grid_data: &[u8]) -> Result<(), Error> {
    self.update_grid_impl(project_id, grid_data).await
  }

  /// Adds a new palette item.
  #[wasm_bindgen(js_name = "addPaletteItem")]
  pub async fn add_palette_item(&self, project_id: &str, palitem_data: &[u8]) -> Result<(), Error> {
    self.add_palette_item_impl(project_id, palitem_data).await
  }

  /// Removes palette items by their indexes.
  #[wasm_bindgen(js_name = "removePaletteItems")]
  pub async fn remove_palette_items(&self, project_id: &str, palindexes: Vec<u32>) -> Result<(), Error> {
    self.remove_palette_items_impl(project_id, palindexes).await
  }

  /// Updates palette display settings.
  #[wasm_bindgen(js_name = "updatePaletteDisplaySettings")]
  pub async fn update_palette_display_settings(&self, project_id: &str, settings_data: &[u8]) -> Result<(), Error> {
    self
      .update_palette_display_settings_impl(project_id, settings_data)
      .await
  }

  /// Sorts palette items by the given criterion.
  #[wasm_bindgen(js_name = "sortPalette")]
  pub async fn sort_palette(&self, project_id: &str, sort_by: &str) -> Result<(), Error> {
    self.sort_palette_impl(project_id, sort_by).await
  }
  ...
  pub async fn reorder_palette_items(
    ...
    self
      .reorder_palette_items_impl(project_id, old_position, new_position)
      .await
  }

  /// Sets the stitch symbol for a palette item.
  #[wasm_bindgen(js_name = "setPaletteItemSymbol")]
  pub async fn set_palette_item_symbol(&self, project_id: &str, symbol_data: &[u8]) -> Result<(), Error> {
    self.set_palette_item_symbol_impl(project_id, symbol_data).await
  }

  /// Updates pattern info.
  #[wasm_bindgen(js_name = "updatePatternInfo")]
  pub async fn update_pattern_info(&self, project_id: &str, info_data: &[u8]) -> Result<(), Error> {
    self.update_pattern_info_impl(project_id, info_data).await
  }

  /// Updates display settings.
  #[wasm_bindgen(js_name = "updateDisplaySettings")]
  pub async fn update_display_settings(&self, project_id: &str, settings_data: &[u8]) -> Result<(), Error> {
    self.update_display_settings_impl(project_id, settings_data).await
  }

  /// Adds a new layer to the pattern.
  #[wasm_bindgen(js_name = "addLayer")]
  pub async fn add_layer(&self, project_id: &str) -> Result<(), Error> {
    self.add_layer_impl(project_id).await
  }

  /// Removes the layer at the given index.
  #[wasm_bindgen(js_name = "removeLayer")]
  pub async fn remove_layer(&self, project_id: &str, layer_index: u32) -> Result<(), Error> {
    self.remove_layer_impl(project_id, layer_index).await
  }

  /// Renames the layer at the given index.
  #[wasm_bindgen(js_name = "renameLayer")]
  pub async fn rename_layer(&self, project_id: &str, layer_index: u32, name: String) -> Result<(), Error> {
    self.rename_layer_impl(project_id, layer_index, name).await
  }
  ...
  pub async fn update_layer_visibility(
    ...
    self
      .update_layer_visibility_impl(project_id, layer_index, visibility_data)
      .await
  }

  /// Moves a layer from `old_position` to `new_position`.
  #[wasm_bindgen(js_name = "moveLayer")]
  pub async fn move_layer(&self, project_id: &str, old_position: u32, new_position: u32) -> Result<(), Error> {
    self.move_layer_impl(project_id, old_position, new_position).await
  }

  /// Updates PDF export options.
  #[wasm_bindgen(js_name = "updatePdfExportOptions")]
  pub async fn update_pdf_export_options(&self, project_id: &str, options_data: &[u8]) -> Result<(), Error> {
    self.update_pdf_export_options_impl(project_id, options_data).await
  }

  /// Sets the reference image.
  #[wasm_bindgen(js_name = "setReferenceImage")]
  pub async fn set_reference_image(&self, project_id: &str, image_data: &[u8]) -> Result<(), Error> {
    self.set_reference_image_impl(project_id, image_data).await
  }

  /// Removes the reference image.
  #[wasm_bindgen(js_name = "removeReferenceImage")]
  pub async fn remove_reference_image(&self, project_id: &str) -> Result<(), Error> {
    self.remove_reference_image_impl(project_id).await
  }

  /// Updates reference image display settings.
  #[wasm_bindgen(js_name = "updateReferenceImageSettings")]
  pub async fn update_reference_image_settings(&self, project_id: &str, settings_data: &[u8]) -> Result<(), Error> {
    self
      .update_reference_image_settings_impl(project_id, settings_data)
      .await
  }

  /// Undoes the last edit or transaction in the pattern with the given ID.
  pub async fn undo(&self, project_id: &str, single: bool) -> Result<(), Error> {
    self.undo_impl(project_id, single).await
  }

  /// Redoes the last undone edit or transaction in the pattern with the given ID.
  pub async fn redo(&self, project_id: &str, single: bool) -> Result<(), Error> {
    self.redo_impl(project_id, single).await
  }

  /// Starts a new transaction in the pattern with the given ID.
  #[wasm_bindgen(js_name = "startTransaction")]
  pub async fn start_transaction(&self, project_id: &str) -> Result<(), Error> {
    self.start_transaction_impl(project_id).await
  }

  /// Ends the current transaction in the pattern with the given ID.
  #[wasm_bindgen(js_name = "endTransaction")]
  pub async fn end_transaction(&self, project_id: &str) -> Result<(), Error> {
    self.end_transaction_impl(project_id).await
  }

  /// Checks if the pattern with the given ID has unsaved changes.
  #[wasm_bindgen(js_name = "hasUnsavedChanges")]
  pub fn has_unsaved_changes(&self, project_id: &str) -> Result<bool, Error> {
    self.has_unsaved_changes_impl(project_id)
  }
}
...
impl EditorWrapper {
  ...
  async fn dispatch(&self, project_id: EmbroiderlyProjectId, action: EditorAction) -> Result<(), Error> {
    ...
    if !events.is_empty() {
      ...
      if let Err(e) = self.persistence.append_journal_entry(project_id, action_bytes).await {
        if let Err(undo_err) = self.run(|editor| editor.undo(&project_id)) {
  ...
  #[tracing::instrument(name = "EditorWrapper::add_pattern", level = "debug", skip_all, ret(Display), err)]
  async fn add_pattern_impl(&self, snapshot: Vec<u8>) -> Result<OpenPatternResult, Error> {
    let embproj: EmbroiderlyProject = borsh::from_slice(&snapshot)?;
    let title = embproj.pattern.info.title.clone();

    let project_id = self.run(|editor| editor.add_pattern(embproj));

    self
      .persistence
      .save_project_entry(project_id, snapshot.clone(), None)
      .await?;

    Ok(OpenPatternResult {
  ...
  async fn open_pattern_impl(&self, file_handle: opfs::FileHandle) -> Result<OpenPatternResult, Error> {
    let file_name = file_handle.name();
    let data = file_handle.read().await?;

    tracing::Span::current().record("file_name", &file_name);

    let embproj = embroiderly_parsers::parse_pattern(&data, &file_name)?;
    let title = embproj.pattern.info.title.clone();
    let title = if title.is_empty() { file_name } else { title };

    let snapshot = borsh::to_vec(&embproj)?;
    let project_id = self.run(|editor| editor.add_pattern(embproj));

    self
      .persistence
      .save_project_entry(project_id, snapshot.clone(), Some(file_handle))
      .await?;
  ...
  async fn open_pattern_from_data_impl(&self, data: &[u8], file_name: &str) -> Result<OpenPatternResult, Error> {
    let embproj = embroiderly_parsers::parse_pattern(data, file_name)?;
    let title = embproj.pattern.info.title.clone();
  ...
  #[tracing::instrument(name = "EditorWrapper::create_pattern", level = "debug", skip_all, ret(Display), err)]
  async fn create_pattern_impl(&self, fabric_data: &[u8]) -> Result<OpenPatternResult, Error> {
    let fabric = borsh::from_slice(fabric_data)?;

    let embproj = EmbroiderlyProject::builder(Pattern::new(fabric)).build();
    let title = embproj.pattern.info.title.clone();
  ...
  async fn load_pattern_impl(&self, project_id: &str) -> Result<Vec<u8>, Error> {
    ...
    self.restore_project(project_id).await?;
    self.run(|editor| {
      if let Some(embproj) = editor.get_pattern(&id) {
        Ok(borsh::to_vec(embproj)?)
      } else {
  ...
  async fn restore_project(&self, project_id: &str) -> Result<(), Error> {
    ...
    let entry = self
      .persistence
      .get_project_entry(id)
      .await?
      .ok_or_else(|| Error::new(ErrorKind::PatternNotFound))?;

    let embproj: EmbroiderlyProject = borsh::from_slice(&entry.data)?;
    self.run(|editor| {
      editor.add_pattern(embproj);
  ...
  async fn save_pattern_impl(&self, project_id: &str, file_handle: Option<opfs::FileHandle>) -> Result<(), Error> {
    ...
    let data = self.run(|editor| {
      if let Some(embproj) = editor.get_pattern(&project_id) {
        Ok(embroiderly_parsers::save_pattern(embproj, format)?)
      } else {
        Err(Error::new(ErrorKind::PatternNotFound))
      }
    })?;
    ...
    self.run(|editor| editor.checkpoint(&project_id))?;
    ...
    emit_events(
      &self.callback,
      [
        EditorEvent::PatternCheckpoint(project_id),
        EditorEvent::PatternSaved(project_id),
      ],
    )?;
  ...
  async fn close_pattern_impl(&self, project_id: &str, force: bool) -> Result<(), Error> {
    ...
    self.run(|editor| {
      ...
      editor.remove_pattern(&project_id);
      Ok(())
    })?;
    self.persistence.remove_project_entry(project_id).await?;
    self.persistence.clear_journal(project_id).await
  }
  ...
  async fn add_stitch_impl(&self, project_id: &str, layer_index: u32, stitch_data: &[u8]) -> Result<(), Error> {
    ...
    if self.run(|editor| {
      editor
        ...
        .is_some_and(|embproj| embproj.pattern.contains_stitch(layer_index, &stitch))
    }) {
      return Ok(());
    }
  ...
  async fn set_palette_item_symbol_impl(&self, project_id: &str, symbol_data: &[u8]) -> Result<(), Error> {
  ...
  async fn undo_impl(&self, project_id: &str, single: bool) -> Result<(), Error> {
    ...
    let (mut events, journal_entry) = if single {
      ...
      let events = self.run(|editor| editor.undo_transaction(&project_id))?;
    ...
    if !events.is_empty() {
      ...
      if let Err(e) = self.persistence.append_journal_entry(project_id, action_bytes).await {
        if single {
          if let Err(redo_err) = self.run(|editor| editor.redo(&project_id)) {
            tracing::error!("Failed to roll back undo after journal failure: {redo_err}");
          }
        } else if let Err(redo_err) = self.run(|editor| editor.redo_transaction(&project_id)) {
          tracing::error!("Failed to roll back undo after journal failure: {redo_err}");
        }
        return Err(e);
      }
    ...
    if !self.run(|editor| editor.has_unsaved_changes(&project_id))? {
      events.push(EditorEvent::PatternCheckpoint(project_id));
    }
  ...
  async fn redo_impl(&self, project_id: &str, single: bool) -> Result<(), Error> {
    ...
    let (mut events, journal_entry) = if single {
      ...
      let events = self.run(|editor| editor.redo_transaction(&project_id))?;
    ...
    if !events.is_empty() {
      ...
      if let Err(e) = self.persistence.append_journal_entry(project_id, action_bytes).await {
        ...
        } else if let Err(undo_err) = self.run(|editor| editor.undo_transaction(&project_id)) {
          tracing::error!("Failed to roll back redo after journal failure: {undo_err}");
        }
        return Err(e);
      }
  ...
  async fn start_transaction_impl(&self, project_id: &str) -> Result<(), Error> {
    ...
    if let Err(e) = self.persistence.append_journal_entry(project_id, action_bytes).await {
      if let Err(end_err) = self.run(|editor| editor.end_transaction(&project_id)) {
  ...
  async fn end_transaction_impl(&self, project_id: &str) -> Result<(), Error> {
    ...
    if let Err(e) = self.persistence.append_journal_entry(project_id, action_bytes).await {
      if let Err(undo_err) = self.run(|editor| editor.undo_transaction(&project_id)) {
...
async fn auto_save_tick(persistence: Rc<PersistenceManager>, callback: js_sys::Function) {
  let project_ids = EDITOR.with(|cell| {
    cell
      .borrow()
      .as_ref()
      .map(embroiderly_editor::Editor::project_ids)
      .unwrap_or_default()
  });
  ...
  tracing::debug!("Processing {} patterns", project_ids.len());
  for project_id in project_ids {
    let dirty = EDITOR.with(|cell| {
      cell
        .borrow()
        .as_ref()
        .and_then(|e| e.has_unsaved_changes(&project_id).ok())
        .unwrap_or(false)
    });
    if !dirty {
      tracing::debug!("Pattern {project_id} is clean, skipping");
      continue;
    }

    let file_handle = match persistence.get_handle(project_id).await {
      Ok(Some(h)) => h,
      Ok(None) => {
        tracing::debug!("No file handle for {project_id}, skipping");
        continue;
      }
      Err(e) => {
        tracing::warn!("Failed to get file handle for {project_id}: {e}");
        continue;
      }
    };
    ...
    let format = match PatternFormat::try_from(file_name.as_str()) {
      Ok(f) => f,
      Err(e) => {
        tracing::warn!("Unknown format for {project_id}: {e}");
        continue;
      }
    };
    ...
    let data = EDITOR.with(|cell| {
      cell
        .borrow()
        .as_ref()
        .and_then(|e| e.get_pattern(&project_id))
        .and_then(|p| embroiderly_parsers::save_pattern(p, format).ok())
    });
    let Some(data) = data else {
      tracing::warn!("Failed to encode {project_id}");
      continue;
    };

    if let Err(e) = file_handle.write(&data).await {
      tracing::warn!("Failed to write {project_id}: {e}");
      continue;
    }

    EDITOR.with(|cell| {
      if let Some(editor) = cell.borrow_mut().as_mut()
        && let Err(e) = editor.checkpoint(&project_id)
      {
        tracing::warn!("Checkpoint failed for {project_id}: {e}");
      }
    });

    if let Ok(action_bytes) = borsh::to_vec(&JournalEntry::Checkpoint)
      && let Err(e) = persistence.append_journal_entry(project_id, action_bytes).await
    {
      tracing::warn!("Failed to journal checkpoint for {project_id}: {e}");
    }
