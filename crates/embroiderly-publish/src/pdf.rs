...
pub fn export_pattern<P: AsRef<std::path::Path>>(
  ...
  if options.monochrome {
    ...
    let frames = embroiderly_image::export::svg::generate_svg(patproj, false, options.frame_options)?;
  ...
  if options.color {
    ...
    let frames = embroiderly_image::export::svg::generate_svg(patproj, true, options.frame_options)?;
    let file_path = file_path.with_file_name(format!(
      "{}.color.{}",
      file_path.file_stem().unwrap_or_default().to_string_lossy(),
      file_path.extension().unwrap_or_default().to_string_lossy()
