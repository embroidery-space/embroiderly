...
impl Editor {
  ...
  pub fn dispatch(&mut self, id: &uuid::Uuid, mut action: EditorAction) -> Result<Vec<EditorEvent>> {
    let patproj = self.patterns.get_mut(id).ok_or(EditorError::PatternNotFound(*id))?;
    let events = action.perform(patproj)?;

    let history = self.histories.get_mut(id).ok_or(EditorError::PatternNotFound(*id))?;
    history.push(action);
  ...
  pub fn has_unsaved_changes(&self, id: &uuid::Uuid) -> Result<bool> {
    let history = self.histories.get(id).ok_or(EditorError::PatternNotFound(*id))?;
